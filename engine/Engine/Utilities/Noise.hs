module Engine.Utilities.Noise where
...
coherentNoise seed (V3 x y z) = clamp noise (-1) 1
    where 
        (ox, oy, oz) = (clampToIntRange x, clampToIntRange y, clampToIntRange z)
        x0           = floor ox
        x1           = x0 + 1
        y0           = floor oy
        y1           = y0 + 1
...
gradientNoise seed fx fy fz ix iy iz = 2.12 * (gx * ox + gy * oy + gz * oz)
    where 
        (gx, gy, gz) =  ( vectorTable ! shiftL i 2
...
data Ridged = Ridged Seed Int Double Double Double (V.Vector Double)
...
computeSpecWeights :: Int -> Double -> V.Vector Double
computeSpecWeights octs lac = V.fromList (computeSpecWeights' octs lac 1)
