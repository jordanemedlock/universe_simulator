...
main = do
    ...
    (tex, _) <- loadTexture "resources/test_texture.png"

    startTime <- getCurrentTime

    mainLoop window startTime (GameState program cubeVAO tex)

    GLFW.terminate

mainLoop :: GLFW.Window -> UTCTime -> GameState -> IO ()
mainLoop window previousTime (GameState program cubeVAO tex) = do
    GLFW.pollEvents
    ...
    if shouldClose 
        then return ()
        else mainLoop window thisTime (GameState program cubeVAO tex)
